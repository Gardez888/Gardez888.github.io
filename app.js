...
function render() {
  ...
  for (var i = 0; i < events.length; i++) {
    ...
    var meta = isOne ? 'Chester, SC - shared book - closes Sat 10:31 AM' : ('Chester, SC - shared book - window closes ' + closes);
...
async function refreshWeather() {
  ...
  for (var i = 0; i < events.length; i++) {
    ...
    try { ev.weather = await fetchPrecipWindow(ev.when); }
    catch (e) { ev.weather = { error: true }; }
  }
  cache = events;
  render();
}
